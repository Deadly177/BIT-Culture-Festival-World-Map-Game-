# Cultural Festival Country Game

This is a web-based interactive country-naming game developed for the **BIT Cultural Festival**.  
The game challenges players to type country names as fast as possible before time runs out.

## 🧑‍💻 Developer
**Muhammadu Haleek **  
Student, Bachelor of Information Technology (BIT)

This project is based on an open-source project by **Bruno Francisco**, licensed under the MIT License.  
I have customized and modified the original project to suit the needs of the cultural festival event.

## 🎮 Game Features
- Start typing country names — **no need to press Enter**  
- The faster you type, the more points you earn  
- Game ends when:
  - You type all countries, or  
  - Time runs out  
- **Time Challenges:**
  - ⚡ Under 1 minute → Name 10 countries  
  - ⏱️ Under 3 minutes → Name 20 countries  
  - 🕔 Under 5 minutes → Name 35 countries  

## 🏆 Prizes
- **1st Place** – Grand Prize  
- **2nd Place** – Runner-up Prize  
- **3rd Place** – Consolation Prize  

## 📜 License
This project includes open-source code licensed under the **MIT License**.  
See the full license in the [`LICENSE`](LICENSE) file.

## 🙏 Credits
- Original project: **Bruno Francisco**  
- Modified & developed for BIT Cultural Festival by **Muhammadu Haleek**

## 📬 Contact
For suggestions or feedback, feel free to reach out.